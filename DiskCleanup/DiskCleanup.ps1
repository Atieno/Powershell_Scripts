...
[array]$files = gci $folder -recurse:$recurse -include $extension | ? {(($_.CreationTime).Date -lt (get-date).AddDays(-$retentionDays).Date) -and -not $_.PSIsContainer}

write-host $folder -recurse:$recurse -include $extension | ? {(($_.CreationTime).Date -lt (get-date).AddDays(-$retentionDays).Date) -and -not $_.PSIsContainer}
